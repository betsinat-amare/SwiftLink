...
export default function SenderDashboard({ route }) {
  ...
  const fetchFlights = useCallback(async () => {
    try {
      const res = await fetch('https://herbackend-4.onrender.com/api/sender/flights', {
        headers: {
  ...
  try {
    const shipmentRes = await fetch('https://herbackend-4.onrender.com/api/sender/shipments', {
      method: 'POST',
      headers: {
    ...
    const paymentRes = await fetch('https://herbackend-4.onrender.com/api/payment/initialize', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
